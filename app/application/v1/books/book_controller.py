...
@books_router_v1.get("/", response_model=List[BookRead])
async def list_books(service: BookService = Depends(get_book_service)):
    return await service.list_books()

@books_router_v1.get("/{book_id}", response_model=BookRead)
...
@books_router_v1.post("/", response_model=BookRead, status_code=status.HTTP_201_CREATED)
...
@books_router_v1.put("/{book_id}", response_model=BookRead)
...
@books_router_v1.delete("/{book_id}", status_code=status.HTTP_204_NO_CONTENT)
